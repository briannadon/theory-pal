...
export function Transport({
  ...
  // Everything past play/bpm collapses behind a disclosure on narrow layouts
  // (see the (max-width: 640px) rules in theme.css); at desktop widths the
  // wrapper divs are `display: contents` and this state is never read.
  const [showMore, setShowMore] = useState(false);

  // The bar's real rendered height feeds .tp-app's bottom padding (--transport-h
  // in theme.css) so content never sits under it — true whether it's one row on
  // desktop, a collapsed primary row on a phone, or the disclosure panel open.
  const footerRef = useRef<HTMLElement>(null);
  useEffect(() => {
    const el = footerRef.current;
    if (!el) return;
    const setHeightVar = () => {
      document.documentElement.style.setProperty('--transport-h', `${Math.ceil(el.getBoundingClientRect().height)}px`);
    };
  ...
  return (
    <footer className="tp-transport" ref={footerRef}>
      <div className="tp-transport__primary">
        <button
          type="button"
        ...
        <button
          type="button"
          className="tp-transport__more-toggle"
          aria-expanded={showMore}
          aria-controls="tp-transport-more"
      ...
      <div
        id="tp-transport-more"
        className={`tp-transport__more${showMore ? ' tp-transport__more--open' : ''}`}
      >
        <button
          type="button"
          className="tp-toggle"
